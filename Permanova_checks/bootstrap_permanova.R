...
nmds_plot <-
  function(species, env, file_prefix = "out") {
    ...
    ggplot(scores2, 
           aes(x = NMDS1, y = NMDS2, color = trt, shape = type)) +
      geom_point(size = 5) +
      scale_shape_manual(values = c(1,15))
    
    ggsave(paste("Permanova_checks/nmds/",file_prefix,".pdf",sep=""), height = 3, width = 4)                   
  }
...
run_grazing_bootstrap <-
  function(sample_threshold = 6) {
    ...
    for (i in sitelist) {
      ...
      wide <-
        subset %>%
        select(site, site_block, exage, block, trt, num, genus_species_use, relcov) %>%
      ...
      if (replication < sample_threshold) {
        ...
        boot_env <-
          data.frame(
            trt = c(rep("G", replication_needed),
